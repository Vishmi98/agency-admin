...
import { EventDataType } from "../event.types";
import { addImages, deleteEvent, deleteEventImage, getEvents, publishEvent } from "../event.service";

import { TableProps } from "@/modules/countries/countries.types";


const columns = [
  { label: "Event Name", key: "year", width: "25%", align: "center" as const },
...
const EventsTable: React.FC<TableProps> = ({ reload }) => {
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<EventDataType | null>(null);
  ...
  const fetchEvents = async (currentPage = page + 1, limit = rowsPerPage) => {
    setIsLoading(true);
    try {
      const response = await getEvents(currentPage, limit);
      if (response?.success) {
        setEvents(response.events);
        setTotalEvents(response.totalEvents || response.events.length); // make sure API returns total
      } else {
        setEvents([]);
        setTotalEvents(0);
      }
    } catch {
      toast.error("Failed to load success events");
      setEvents([]);
      setTotalEvents(0);
    } finally {
      setIsLoading(false);
  ...
  const handlePublishConfirm = async () => {
    ...
    try {
      const newStatus = !selectedEvent.isPublish;
      const res = await publishEvent(selectedEvent.id, newStatus);

      if (res.success) {
        toast.success(
          `Event ${newStatus ? "published" : "unpublished"}`
        );
        fetchEvents();
      } else {
  ...
  const handleDeleteImageConfirm = async () => {
    ...
    try {
      const res = await deleteEventImage(
      ...
      return (
        <TableRow>
          <TableCell colSpan={columns.length} align="center">
            No success events found
          </TableCell>
        </TableRow>
      );
    }

    return events.map((award, index) => (
      <React.Fragment key={award.id}>
        ...
        >
          {/* YEAR */}
          <TableCell align="center">{award.eventName}</TableCell>
    ...
    <Box>
      ...
      >
        <DialogTitle sx={{ fontWeight: "bold" }}>
          Confirm {selectedEvent?.isPublish ? "Unpublish" : "Publish"} award
        </DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to{" "}
            {selectedEvent?.isPublish ? "Unpublish" : "Publish"} this award?
          </Typography>
        </DialogContent>
