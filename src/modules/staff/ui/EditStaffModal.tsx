...
import { EditStaffModalProps, StaffType } from "../staff.types";
import { addStaffValidationSchema } from "../staff.utils";
import { getBranches, getGenders, getTitles, updateStaff } from "../services/staff.services";
...
const EditStaffModal: React.FC<EditStaffModalProps> = ({ isOpen, onClose, initialValues, reloadData }) => {
    ...
    const handleSubmit = async (values: StaffType, { resetForm }: FormikHelpers<StaffType>) => {
        try {
            const response = await updateStaff(values.id, values);
