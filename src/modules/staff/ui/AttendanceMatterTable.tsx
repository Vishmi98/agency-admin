...
import { updateAttendanceMatterStatus } from '../services/staff.services';
...
const columns: TableColumnType[] = [
  ...
  { label: "Attendance Matter", width: "20%", align: "right" as const },
];

const AttendanceMatterTable: React.FC<StaffTableProps> = ({
  totalRows,
  staffs,
  isLoading,
  page,
  ...
  const handleUpdateStatus = async (staffId: number, currentValue: boolean) => {
    ...
    try {
      const response = await updateAttendanceMatterStatus(staffId, !currentValue);
    ...
    return staffs.map((member: StaffDataType, index: number) => (
      ...
      >
        ...
        <TableCell align="right">
          <Switch
            checked={member.isAttendanceMatter}
            onChange={() => handleOpenModal(member.id, member.isAttendanceMatter)}
            size="small"
            color="primary"
...
export default AttendanceMatterTable;