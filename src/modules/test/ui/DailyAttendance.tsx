...
import { fetchAttendance } from "../service/test.service";


const workingHours = Array.from({ length: 12 }, (_, i) => {
    const hour = 7 + i; // Start at 07:00 and increment by 2 hours
...
const DailyAttendance: React.FC<DailyAttendanceProps> = ({ selectedDate: initialDate }) => {
    ...
    useEffect(() => {
        const loadAttendance = async () => {
            setLoading(true);
            const { data } = await fetchAttendance(selectedDate.toISOString().split("T")[0]);
