...
import { AddedExtraPaymentsType } from "../../invoice.types";
...
const AddedExtraPayments = ({
    ...
    selectedInvoice,
    removeExtraPayment,
}: AddedExtraPaymentsType) => {

    return (
        <Box sx={{ marginTop: "16px" }}>
            <Box>
                <Typography variant="h6" pb={1.2}>Added Extra Payments:</Typography>
                <Stack
                    sx={{
                        width: "100%",
                        flexDirection: 'row',
                ...
                {selectedInvoice && selectedInvoice.extraPaymentInfo?.length > 0 && selectedInvoice.extraPaymentInfo.map((payment) => (
                    <Stack key={payment.id}
                        ...
                        <Stack sx={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                            <Typography variant="body2" mt={1}>
                                LKR  {payment.amount.toLocaleString('en-US')}
                            </Typography>
                        </Stack>
                    </Stack>
                ))}
